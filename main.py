...
while not gameExit:
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYUP:
			if event.key == pygame.K_SPACE:
				missiles1.append(Missile1(gameDisplay, space_ship.x, space_ship.y-block_height, timer))
			if event.key == pygame.K_s:
				missiles2.append(Missile2(gameDisplay, space_ship.x, space_ship.y-block_height, timer))
	if timer%(10*fps) == 0:
		x , y = spawn_alien(aliens)
		aliens.append(Alien(gameDisplay, x, y, timer+(8*fps)))
	...
	while i<len(missiles2):
		j=0
		while j<len(aliens):
			if detect_collision(aliens[j], missiles2[i]) == True:
